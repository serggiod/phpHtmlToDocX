...
class Html
{
    ...
    /*public static function addHtml($element, $html, $fullHTML = false, $preserveWhiteSpace = true, $options = null)
    {
         ...
         * which could be applied when such an element occurs in the par
         * seNode function.
         */
        ...
        $node  = $this->xpath->query("/html/body/*");

        self::parseNode($node->item(0), $element);
    }*/

    public static function addHtml($element, $dom, $xpath, $WordStyle)
    {
        $node = $dom->getElementsByTagName('body');
        self::$xpath = $xpath;
    ...
    protected static function parseNode($node, $element, $styles = array(), $data = array())
    {
        ...
        if (isset($nodes[$node->nodeName])) {
            ...
            $newElement = call_user_func_array(array('Html', $method), $arguments);
    ...
    private static function parseRow($node, $element, &$styles)
    {
        ...
        foreach($node->attributes as $attribute){
            switch($attribute->name)
            {
                case 'align':
                    $a = $attribute->value;
                    if($a=='center')   $a = \PhpOffice\PhpWord\SimpleType\Jc::CENTER;
                    if($a=='middle')   $a = \PhpOffice\PhpWord\SimpleType\Jc::CENTER;
                    if($a=='left')     $a = \PhpOffice\PhpWord\SimpleType\Jc::LEFT;
                    if($a=='right')    $a = \PhpOffice\PhpWord\SimpleType\Jc::RIGHT;
                    if($a=='justify')  $a = \PhpOffice\PhpWord\SimpleType\Jc::JUSTIFY;
                    if($a=='top')      $a = \PhpOffice\PhpWord\SimpleType\JcTable::START;
                    if($a=='baseline') $a = \PhpOffice\PhpWord\SimpleType\JcTable::END;
        ...
        foreach ($node->attributes as $attribute) {
            switch ($attribute->name) {
                ...
                case 'width':
                    ...
                    $style['width'] = Converter::pixelToTwip(intval($width));
                    $style['unit'] = \PhpOffice\PhpWord\Style\Image::UNIT_PX;
                    break;
                case 'height':
                    $height = $attribute->value;
                    $style['height'] = Converter::pixelToTwip(intval($height));
                    $style['unit'] = \PhpOffice\PhpWord\Style\Image::UNIT_PX;
                    break;
                case 'style':
